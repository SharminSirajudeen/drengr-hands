...
enum Commands {
    ...
    /// Inspect the business events Drengr auto-discovers from your app's traffic.
...
async fn main() -> anyhow::Result<()> {
    ...
    // Capture every user in our backend (keyless included), once per invocation.
    // Fire-and-forget. Skipped for: uninstall (forgets this machine); and
    // mcp/demo/run, which already register the trial inline (with the gating
    match cli.command {
        Commands::Mcp {
            device,
            format,
        ...
        Commands::Demo { app, task } => {
            ...
            fn print_device_help() {
                eprintln!(
                    "  Couldn't find or boot a device. Start one, then run `drengr demo` again:"
                );
                eprintln!();
                eprintln!("    • iOS (macOS):  xcrun simctl boot \"iPhone 15\"   (or open a simulator in Xcode)");
                eprintln!("    • Android:      launch an emulator in Android Studio, or `emulator -avd <name>`");
                eprintln!();
            }
            ...
            match outcome {
                Ok(r) if r.success => {
                    ...
                    if find_mcp_configs(&dirs::home_dir().unwrap_or_default()).is_empty() {
                        ...
                        eprintln!(
                            "    • Your MCP client is already wired — just ask it to drive an app."
                        );
                    }
                    eprintln!("    • Run your own task:  drengr run --app <pkg> --task \"…\"");
                    eprintln!("    • Optional:           drengr login   (links machines to a free account)");
                }
                Ok(r) => {
                    eprintln!(
                    ...
                    eprintln!("  (Small local models wobble on longer tasks — a frontier model is steadier.)");
                }
                Err(e) => {
                    eprintln!("  The demo hit an error: {}", e);
                }
            }
            // Give the fire-and-forget outcome event time to leave before exit.
        ...
        Commands::Run {
            ...
            let transport: Box<dyn drengr_hands::transport::DeviceTransport> =
                if let Some(ref cloud_provider) = cloud {
                    ...
                    let detected = drengr_hands::transport::detect::auto_select_device()
                        .await
                        .map_err(|e| {
                            eprintln!("Error: {}", e);
                            std::process::exit(2);
        ...
        Commands::Setup {
            ...
            // Resolve client — interactive if not specified
                ...
                if std::io::stdin().is_terminal() {
                    ...
                    match trimmed {
                        ...
                        _ => {
                            if !trimmed.is_empty() {
                                println!(
                                    "  Invalid choice: \"{}\". Showing raw config.\n",
                                    trimmed
                                );
                            }
                            "raw".to_string()
            ...
            let home_dir = match dirs::home_dir() {
                ...
                None => {
                    if write {
                        eprintln!(
                            "  Cannot determine home directory. Use --client without --write."
                        );
                        std::process::exit(1);
                    }
                    "~".to_string()
        ...
        Commands::Doctor {
            ...
            // Check simctl (optional — macOS only)
            let xcrun_bin = drengr_hands::transport::simctl::resolve_xcrun();
            ...
            if !any_cloud {
                println!(
                    "  [–] Cloud devices          not configured (set BROWSERSTACK_* or SAUCE_*)"
                );
            }

            println!("\n  ─────────────────────────────────────────────");

            // Four-state output with concrete next steps
            if !adb_ok && !simctl_ok {
                // BLOCKED
                ...
                if adb_ok {
                    println!(
                        "  Android: connect a device (USB debugging on) or start an emulator."
                    );
                } else {
            ...
            // Non-blocking update check at the end of doctor
        ...
        Commands::Uninstall => {
            ...
            // Find MCP configs that reference drengr
            ...
            println!(
                "  Project-local MCP configs (.mcp.json / .cursor/mcp.json / .vscode/mcp.json)"
            );
        ...
        Commands::Key { action } => {
            ...
            match action {
                None => {
                    ...
                    println!("  Your Drengr license is managed via: drengr login\n");
